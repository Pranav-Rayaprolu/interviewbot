#copy of app3.py with modifaction to no of questions that can  be asked.
...
"""
    ...
    def respond(self, user_input, company=None, domain=None):
        ...
        if current_index < 3:
            ...
            print("No more questions. going to the dashboard")
            return self.complete_interview()
...
class Interview:
  def __init__(self):
    self.state = {"evaluation_metrics": {}}  # Placeholder for evaluation data

  def complete_interview(self):
    df = pd.DataFrame(self.state["evaluation_metrics"])
    print("Interview completed. Evaluation DataFrame:")
    print(df)
    self.state["evaluation_metrics"] = {}  # Reset evaluation data
    return self.create_dashboard(df.copy())  # Pass a copy to avoid modifying original
  ...
  def create_dashboard(self, df):
    ...
    for metric in df.columns:
      plt.bar(df.index, df[metric], label=metric)
