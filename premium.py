...
class InterviewBot:
    ...
    def verify_job_position(self, company, job_position):
        df = self.dataset[['company_name', 'Job_Position']].fillna('')  # Replace NaN with empty string
        available_positions = df.apply(lambda x: (x['company_name'].lower(), x['Job_Position'].lower()), axis=1).tolist()
...
"""
    ...
    def respond(self, user_input, company=None, domain=None):
        ...
        if user_input.lower() == "Exit" or "exit":
