...
""")
    ...
    def evaluate_all_answers(self):
        ...
        # print(all_answers) #this prints what user answer is and what  original answer is
        print(response)
        metrics_list = []

            # metrics_list = [ast.literal_eval(m) for m in response.strip().split('\n')]
        df_data = []
        for i, metrics_dict in enumerate(metrics_list, start=1):
                    df_data.append({
                        "Accuracy": metrics_dict["Accuracy"],
                        "Clarity": metrics_dict["Clarity"],
