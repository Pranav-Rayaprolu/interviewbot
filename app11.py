# //this is a really good stable version which we can scale. i am trying to implement free functionality here
...
class InterviewBot:
    ...
    def free_verify_job_position(self, company, job_position):
        free_companies = ["Wipro","Infosys", "TCS" ,"HCL Technologies","KPIT Tech"]
        free_positions = ["Machine Learning", "NLP", "Python" ]
        available_positions = self.dataset[['company_name'.isin(free_companies), 'Job_Position'.isin(free_positions)]].apply(lambda x: (x['company_name'].lower(), x['Job_Position'].lower()), axis=1).tolist()
...
"""
    ...
    def respond(self, user_input, company=None, domain=None):
        if company and domain and not self.is_job_position_verified:
            is_verified, first_question = self.free_verify_job_position(company, domain)
            if is_verified:
        ...
        questions = self.state.get_context("questions")
        if current_index < 3:#this will generate only 3 questions 
