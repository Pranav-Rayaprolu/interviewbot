...
class InterviewBot:
    ...
    def get_next_question(self):
        ...
        if current_index < len(questions):
            ...
            self.text_to_speech(question)  # Ask the question via TTS
            return question
        else:
            return None
    ...
    def text_to_speech(self, text):
        ...
        tts.save(filename)
        os.system(f"start {filename}")  # For Windows, use `afplay` on macOS or `mpg321` on Linux
    ...
    def respond(self, user_input=None, company=None, domain=None):
        ...
        if current_index < len(questions):
            ...
            if not user_input:
                user_input = self.speech_to_text()  # Capture the user's answer via STT
...
def start_interview():
    ...
    response = bot.respond(company=company, domain=domain)
    return jsonify({'response': response})

@app.route('/capture_response', methods=['POST'])
def capture_response():
    user_input = bot.speech_to_text()  # Capture the response via STT
    response = bot.respond(user_input=user_input)
    return jsonify({'response': response})
