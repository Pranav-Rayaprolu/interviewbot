...
class InterviewBot:
    ...
    def verify_job_position(self, user_input):
        parts = user_input.split(',')
        if len(parts) != 2:
            return False
        company, job_position = parts[0].strip(), parts[1].strip()
    ...
    def respond(self, user_input):
        ...
        if not self.is_job_position_verified:
            if self.verify_job_position(user_input):
